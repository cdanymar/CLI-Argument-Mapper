...
namespace parse
{
    ...
    /// @returns Copy of the original text.
    template <>
    [[nodiscard]] constexpr auto to(const std::string_view text) -> std::expected<std::string, failure>
    {
        return std::string(text.cbegin(), text.cend());
    }

    /// @returns View-copy of the original text.
    template <>
    [[nodiscard]] constexpr auto to(const std::string_view text) -> std::expected<std::string_view, failure>
    {
        return text;
    }
}
